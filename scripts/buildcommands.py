...
def read_config_version():
    """Read CONFIG_VERSION from .config file"""
    config_file = os.getenv("KCONFIG_CONFIG", ".config")
    if not os.path.exists(config_file):
        return "IDM"  # Default if .config doesn't exist
    try:
        ...
        pass
    return "IDM"  # Default on error

class HandleVersions:
    def __init__(self):
        self.ctr_dispatch = {}
        self.toolstr = self.version = ""
        self.config_version = read_config_version()
    def update_data_dictionary(self, data):
